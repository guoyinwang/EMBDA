...
struct pair_table *createTable(int size){
    struct pair_table *t = (struct pair_table*)malloc(sizeof(struct pair_table));
    t->size = size;
...
int hashCode(struct pair_table *t,int key){
    if(key<0)
        return -(key%t->size);
    return key%t->size;
}
void insert(struct pair_table *t,int key,int val){
...
int lookup(struct pair_table *t,int key){
    int pos = hashCode(t,key);
    struct node *list = t->list[pos];
    struct node *temp = list;
    while(temp){
...
char word_pair_file[MAX_STRING];
...
int ReadWordPairs(char *word, FILE *fin) {
  int i, j;
  ...
  wp[loc].key = loc;
  wp[loc].ptab = createTable(num);
  for (i = 0; i < num; ++i) {
    ReadVocabWord(word, fin);
    j = SearchVocab(word);
    insert(wp[loc].ptab,j,1);
  }
  return 1;
}

int SearchWordPairsInt(int word1, int word2) {
	if(wp[word1].key==-1) return -1;
	return lookup(wp[word1].ptab, word2);
}

int SearchWordPairs(char *word1, char *word2) {
	int i, j;
	i = SearchVocab(word1);
	if(wp[i].key==-1)
		return -1;
	j = SearchVocab(word2);
	return lookup(wp[i].ptab, j);
}

void ReadWordPairsFromFile() {
	long long a;
	long long wp_size = 0;
	char *word = (char *)calloc(MAX_STRING, sizeof(char));
  FILE *fi = fopen(word_pair_file, "r");
  for (a = 0; a < vocab_size; a++) {
    wp_size += ReadWordPairs(word, fi);
  }
  free(word);
  fclose(fi);
  printf("Word pair size: %lld\n", wp_size);
}
...
void InitNet() {
  ...
  if (negative>0) {
    ...
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1neg[a * layer1_size + b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / layer1_size; //0;
    for (a = 0; a < vocab_size; a++) for (b = 0; b < window_layer_size; b++)
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (type == 0) { //train skip-gram with domain indicator
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (ds > 0) {
          ...
          ditag = SearchWordPairsInt(word, last_word);
          l2 = word * layer1_size;
          f = 0;
    ...
    } else if(type==1) {  //train countinuous bag-of-words with domain attention
      // in -> hidden
      cw = 0;
      dasum = 0;
      l1 = word * layer1_size;
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        for (c = 0; c < layer1_size; c++) neu1[c] += syn0[c + last_word * layer1_size] * datt;
        cw++;
      }
      if (cw) {
      	//// attention normalization
        for (c = 0; c < layer1_size; c++) neu1[c] /= dasum; //cw;
...
void TrainModel() {
  ...
  wp = (struct word_pair *)calloc(vocab_size, sizeof(struct word_pair));
  if (word_pair_file[0] != 0) ReadWordPairsFromFile();
...
int main(int argc, char **argv) {
  int i;
  if (argc == 1) {
    printf("WORD VECTOR estimation toolkit v 0.1d\n\n");
    printf("Options:\n");
    ...
    printf("\t-word-pair-file <file>\n");
    printf("\t\tThe word pair will be read from <file>\n");

    printf("\t-type <int>\n");
    printf("\t\tType of embeddings (0 for skip-gram with domain indicator, 1 for continuous bag-of-words with domain attention)\n");
    printf("\t-cap <int>\n");
    printf("\t\tlimit the parameter values to the range [-50, 50]; default is 0 (off)\n");
    printf("\nExamples:\n");
    printf("./embda -train data.txt -read-vocab vocab.txt -word-pair-file wordpair.txt -output vec.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -type 0 -iter 3\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-word-pair-file", argc, argv)) > 0) strcpy(word_pair_file, argv[i + 1]);
